...
/// membership in an active season — the admin must remove them from their
/// flight first. Works regardless of IsActive, since deactivated players
/// remain sub-eligible.
/// </summary>
...
public sealed class SetPlayerSubstituteCommandHandler : IRequestHandler<SetPlayerSubstituteCommand, Result<PlayerDto>>
{
    ...
    public async Task<Result<PlayerDto>> Handle(SetPlayerSubstituteCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            var hasActiveFlightMembership = player.FlightMemberships.Any(fm => fm.Season.IsActive);
            if (hasActiveFlightMembership)
                return Result<PlayerDto>.Fail(
                    "This player is still assigned to a flight for the active season. Remove them from their flight before marking them as a substitute.");
        }
